...
# ── List on Vinted (reuses stored photos — no upload UI yet) ───────────────

@router.post("/list-vinted")
async def list_vinted(body: dict, user: dict = Depends(get_current_user)):
    ...
    purchase_price = float(item.get("purchase_price") or 0)
    if live_price > 0:
        price = max(round(live_price * 0.95, 2), config.EBAY_MIN_PRICE_GBP)
    elif purchase_price > 0:
        price = purchase_price
    else:
        return {"success": False, "error": "Item has no recorded price"}
    ...
    if stored_urls:
        tmp_paths = await _download_images_to_temp(item_id, stored_urls)
    if not tmp_paths:
        return {"success": False, "error": "No photos available — add a photo to this item first"}

    condition = lister_vinted.INVENTORY_CONDITION_MAP.get(item.get("condition") or "", "good")

    try:
        result = await lister_vinted.list_item_on_vinted(
            item_name   = body.get("title") or item["card_name"],
            price_gbp   = price,
            image_paths = tmp_paths,
            condition   = condition,
        ...
        if result.success:
            await db.edit_item(user["id"], item_id, "vinted_listed", "Yes")
            await db.edit_item(user["id"], item_id, "vinted_listing_url", result.listing_url or "")
        return {
            "success":     result.success,
            "listing_url": result.listing_url,
            "price":       price,
...
# ── List on Vinted with fresh photo uploads (listings-page drawer) ─────────

@router.post("/list-vinted-form")
async def list_vinted_form(
    item_id:      int                 = Form(...),
    title:        str                 = Form(""),
    ...
    """List on Vinted with user-uploaded photos (same upload pattern as /list-ebay)."""
    try:
        item = await db.get_item(user["id"], item_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    ...
    for upload in uploads:
        safe = "".join(c for c in (upload.filename or "img") if c.isalnum() or c in "._-")
        dest = Path(config.TEMP_IMAGES_DIR) / f"vinted_{item_id}_{safe}"
        content = await upload.read()
        dest.write_bytes(content)
        tmp_paths.append(dest)
    ...
    if price > 0:
        ...
        listing_price = quick_price
    elif live_price > 0:
        listing_price = max(round(live_price * 0.93, 2), config.EBAY_MIN_PRICE_GBP)
    else:
        for p in tmp_paths:
            p.unlink(missing_ok=True)
        return {"success": False, "error": "Item has no recorded price"}

    vinted_condition = lister_vinted.INVENTORY_CONDITION_MAP.get(
        condition or item.get("condition") or "", "good"
    )
